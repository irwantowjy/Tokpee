  ...
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 " id="sidenav-main">
      ...
      <div class="collapse navbar-collapse  w-auto  max-height-vh-100 h-100" id="sidenav-collapse-main">
          <ul class="navbar-nav">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="HomepagePilihan.php">
                <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                    <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 576 512"><!--! Font Awesome Free 6.4.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path d="M575.8 255.5c0 18-15 32.1-32 32.1h-32l.7 160.2c0 2.7-.2 5.4-.5 8.1V472c0 22.1-17.9 40-40 40H456c-1.1 0-2.2 0-3.3-.1c-1.4 .1-2.8 .1-4.2 .1H416 392c-22.1 0-40-17.9-40-40V448 384c0-17.7-14.3-32-32-32H256c-17.7 0-32 14.3-32 32v64 24c0 22.1-17.9 40-40 40H160 128.1c-1.5 0-3-.1-4.5-.2c-1.2 .1-2.4 .2-3.6 .2H104c-22.1 0-40-17.9-40-40V360c0-.9 0-1.9 .1-2.8V287.6H32c-18 0-32-14-32-32.1c0-9 3-17 10-24L266.4 8c7-7 15-8 22-8s15 2 21 7L564.8 231.5c8 7 12 15 11 24z"/></svg>
                </div>
  ...
  <main class="main-content position-relative max-height-vh-100 h-100 mt-1 border-radius-lg ">
    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
      ...
      <nav aria-label="breadcrumb">
          ...
          <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Homepage - Pilihan</li>
          </ol>
          <h6 class="font-weight-bolder mb-0">Homepage - Pilihan</h6>
      </nav>
      ...
      <div class="main-panel">
        <div class="content-wrapper">
          <div class="card" style="padding:10px 24px;">
            <div class="card-body" >
              <h4 class="card-title">Kategori Pilihan Images</h4>             
              <form class="forms-sample" method="post" enctype="multipart/form-data">
                <div class="form-group row">
                  <label for="profileImage" class="col-sm-3 col-form-label">Add Image</label>
                  <div class="col-sm-6">
                    <input type="file" accept="image/*" name="imageKategori" class="form-control" id="imageKategori" required>
                  </div>
                </div>
          ...
          <?php
            if(isset($_POST['submitAdd'])){
              if(!empty($_FILES['imageKategori']['name'])){
                require_once('../php/imageFunction.php');
                $pict = $_FILES['imageKategori'];
                $image = uploadImage('Kategori', $pict);
                $result = "INSERT INTO tbl_admin_kategori (kategori_image) VALUES ('$image')";
                  ...
                  ?>
                    <script>
                      ...
                      text: 'Image Has Been Added',
                          ...
                          window.location = "homepagePilihan.php";
                      })
                    </script>
                  <?php
                }
          ...
          <div class="card-body">
            <h4 class="card-title" style="padding:12px 24px;">Images Data</h4>    
              <div class="table-responsive table-wrapper"  >
                <table class="table display" id="table">
                  ...
                  <?php
                    $i = 1;
                    $result = $conn->query("SELECT * FROM tbl_admin_kategori");
                    while($row = $result->fetch_assoc()) {
                      $id = $row['kategori_id'];
                      $image = $row['kategori_image']; 
                      echo "<tr>";
                      echo "<td style='padding:12px 24px;'>{$i}</td>";
                      echo "<td style='padding:12px 24px;'><img style='width:100px' src='../../UploadImage/Kategori/{$image}'></td>";
                      ?>
                      <form method="post">
                        <td>
                        ...
                        <tr>
                          <td style='padding:12px 24px;'>
                            <form method="post" enctype="multipart/form-data">
                              <input type="hidden" name="id" value="<?= $id ?>">
                              <input type="file" class="form-control" id="imageKategoriUpdate" name="imageKategoriUpdate" style="margin-bottom:10px" required>
                              <input class='btn-primary' style='padding:5px 10px;' type='submit' name='updateData' value='Update'>
                            </form>
                          </td>
                        </tr>
                        <?php                        
                    ...
                    if (isset($_POST['updateData'])) {
                      ...
                      if (!empty($_FILES['imageKategoriUpdate']['name'])) {
                        require_once('../php/imageFunction.php');
                        $pict = $_FILES['imageKategoriUpdate'];
                        $image = uploadImage('Kategori', $pict);
                    
                        // Update the image in the database
                        $result = "UPDATE tbl_admin_kategori SET kategori_image = '$image' WHERE kategori_id = '$id'";
                        if ($conn->query($result)) {
                          ?>
                          <script>
                            Swal.fire({
                  ...
                  if (isset($_POST['delete'])) {
                    $id = $_POST['id'];
                    $result = "DELETE FROM tbl_admin_kategori WHERE kategori_id = '$id'";
                    if ($conn->query($result)) {
                      ?>
                      <script>
                        Swal.fire({
