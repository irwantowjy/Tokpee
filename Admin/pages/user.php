    ...
    <div class="container-scroller">
      <div class="main-panel">
        <div class="content-wrapper">                   
          ...
          <div class="card-body">
            <h4 class="card-title" style="padding:12px 24px;">User Account</h4>    
              <div class="table-responsive table-wrapper"  >
                <table class="table display" id="table">
                  ...
                  <tbody>
                  <?php
                    $sql = "SELECT * FROM tbl_akun_user";
                    $result = $conn -> query($sql);
                    $i = 1;
                    while($row = $result->fetch_assoc()) {
                      echo "<tr>";
                      echo "<td style='padding:12px 24px;'>{$row['email']} </td>";
                      echo "<td style='padding:12px 24px;'>{$row['username']} </td>";
                      echo "<td style='padding:12px 24px;'>{$row['user_password']} </td>";
                      $image= $row['user_profile']; 
                      if (!empty($image) && file_exists("../../UploadImage/Profile/{$image}")) {
                        echo "<td><img style='width:100px' src='../../UploadImage/Profile/{$image}'></td>";
                      } else {
                        echo "<td></td>";
                      }
                      echo "<td style='padding:12px 24px;'>{$row['no_hp']} </td>";
                      echo "<td style='padding:12px 24px;'>{$row['jenis_kelamin']} </td>";
                      echo "<td style='padding:12px 24px;'>{$row['alamat']} </td>";
                      echo "<td style='padding:12px 24px;'>{$row['date_created']} </td>";
                      $id = $row['user_id'];
                      ?>
              ...
              <div class="card-body" >
                ...
                <form class="forms-sample" method="post" action=""  enctype="multipart/form-data">
                  <div class="form-group row">
                    ...
                    <div class="col-sm-6">
                      <input type="text" required name="editEmail" class="form-control" id="email" placeholder="Email" value="<?php if(isset($tmp)) echo $tmp['email']; ?>">
                    </div>
                  </div>
                  <div class="form-group row">
                    <label for="username" class="col-sm-3 col-form-label">Username</label>
                    <div class="col-sm-6">
                      <input type="text" required name="editUsername" class="form-control" id="username" placeholder="Username" value="<?php if(isset($tmp)) echo $tmp['username']; ?>">
                    </div>
                  </div>
                  <div class="form-group row">
                    <label for="username" class="col-sm-3 col-form-label">Password</label>
                    <div class="col-sm-6">
                      <input type="text" required name="changePassword" class="form-control" id="password" placeholder="Password" value="<?php if(isset($tmp)) echo $tmp['user_password']; ?>">
                    </div>
                  </div>
                  <div class="form-group row">
                    <label for="profileImage" class="col-sm-3 col-form-label">Profile Picture</label>
                    <div class="col-sm-6">
                      <?php 
                        if(isset($tmp)){
                          $photo = $tmp['user_profile'];
                          $id = $_POST['id'];
                          echo "<input type='hidden' name='id' value='$id'>";
                          echo "<img style='width: 150px;  margin-right: 10px;' src='../../UploadImage/Profile/$photo'>";
                        } 
                        ?> 
                      <input type="file" accept="image/*" name="editProfilePicture" class="" id="profilePicture">
                    </div>
                  </div>
                  <div class="form-group row">
                    <label for="username" class="col-sm-3 col-form-label">Phone Number</label>
                    <div class="col-sm-6">
                      <input type="text" required name="editPhoneNumber" class="form-control" id="phoneNumber" placeholder="Phone Number" value="<?php if(isset($tmp)) echo $tmp['no_hp'];?>">
                    </div>
                  </div>
                  <div class="form-group row">
                    <label for="username" class="col-sm-3 col-form-label">Gender</label>
                    <div class="col-sm-6">
                      <input type="text" required name="editGender" class="form-control" id="gender" placeholder="Gender" value="<?php if(isset($tmp)) echo $tmp['jenis_kelamin'];?>">
                    </div>
                  </div>
                  <div class="form-group row">
                    <label for="username" class="col-sm-3 col-form-label">Address</label>
                    <div class="col-sm-6">
                      <input type="text" required name="editAddress" class="form-control" id="address" placeholder="Address" value="<?php if(isset($tmp)) echo $tmp['alamat'];?>">
                    </div>
                  </div>
        ...
        <?php
          if(isset($_POST['submitEdit'])){
            ...
            $pict = $_FILES['editProfilePicture'];
            $phone = $_POST['editPhoneNumber'];
            $gender = $_POST['editGender'];
            $address = $_POST['editAddress'];
            updateUser($id, $email, $user, $pass, $pict, $phone, $gender, $address);
          }
        ?>
  <!--   Core JS Files   -->
